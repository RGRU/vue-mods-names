...
**Преимущества**
1. Используя плагин, каждый компонент вашего приложения получает возможность принимать модификаторы через props: mods. (Вам больше не нужно добавлять это поведение в отдельные компоненты)
